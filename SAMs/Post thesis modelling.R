...
#OLD SCRIPT without Detrended Bathy 
...
zstar  <- st_as_stars(b)   # convert the bathymetry raster to a stars object
detre <- detrend(zstar, parallel = 8)      #calculate detrended bathy on your stars raster
detre <- as(object = detre, Class = "Raster")   # convert it back to raster
names(detre) <- c("detrended", "lineartrend")   # set the names of the raster
plot(detre)
...
ders <- stack(b,s,a,r,t,f,detre)
names(ders) <- c("depth", "slope",  "aspect" ,  "roughness","tpi", "flowdir", "detrend")
